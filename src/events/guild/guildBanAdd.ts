...
export default class extends Event {
	...
	public async run(guild: Guild, target: User): Promise<void> {
		...
		await infractionsManager.createInfraction(guild.id, "Manual Ban", true, <string>reason, target, executor);
		const infID: number = await infractionsManager.getLatestInfraction(guild.id, executor.id, target.id, "Manual Ban")
		await loggingManager.sendModAction(this.client, guild.id, "manually banned", target, executor, <string>reason, infID)
	}
}
