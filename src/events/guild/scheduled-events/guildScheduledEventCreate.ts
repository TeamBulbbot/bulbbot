...
export default class extends Event {
    ...
    async run(scheduledEvent: GuildScheduledEvent) {
        ...
        if(Date.now() < createdTimestamp + 3000) {
            msg = await this.client.bulbutils.translate("event_guild_scheduled_event_create_moderator", scheduledEvent.guild?.id, {
                scheduledEvent,
                moderator: executor,
            });
        }
        ...
        if(!msg)
            msg = await this.client.bulbutils.translate("event_guild_scheduled_event_create", scheduledEvent.guild?.id, {
                scheduledEvent,
            });
