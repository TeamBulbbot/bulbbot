...
module.exports = {
	set: async (message, guild, category, user, value, timeout = 10000) => {
		...
		setTimeout(function () {
			if (cache[guild] === undefined ||
				cache[guild][category] === undefined ||
				cache[guild][category][user] === undefined) return;

			cache[guild][category][user] = cache[guild][category][user] - value;
			if (cache[guild][category][user] <= 0) delete cache[guild][category][user];
			if (Object.keys(cache[guild]["messages"]).length === 0 &&
				Object.keys(cache[guild]["mentions"]).length === 0)
				delete cache[guild];
		}, timeout);
	},
