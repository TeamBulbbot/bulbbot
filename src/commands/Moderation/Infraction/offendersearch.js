...
module.exports = {
	Call: async (client, message, args) => {
		...
		try {
			...
			return message.channel.send(await client.bulbutils.translate("global_user_not_found"));
		}
		...
		if (pages.length === 0) {
			return message.channel.send(await client.bulbutils.translate("infraction_list_not_found"));
		}
