...
export default class ArchiveUser extends ApplicationSubCommand {
	...
	public async run(interaction: CommandInteraction): Promise<void> {
		...
		await writeFile(`${rootDir}/files/archive-data-${interaction.guild?.id}-${user.id}.txt`, archive);
		...
		return void (await interaction.followUp({
			...
			files: [
				{
					attachment: `${rootDir}/files/archive-data-${interaction.guild?.id}-${user.id}.txt`,
					name: "archive.txt",
				},
			],
		}));
