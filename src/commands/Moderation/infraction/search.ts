...
export default class extends SubCommand {
	constructor(...args) {
		...
		super(...args, {
			...
			maxArgs: 1,
			argList: ["user:User"],
			usage: "!inf search <user>",
		});
	}

	async run(message: Message, parent: Command, args: string[]): Promise<void | Message> {
		...
		if (pages.length === 0) return await message.channel.send(await this.client.bulbutils.translateNew("infraction_search_not_found", message.guild?.id, { target: user }));
