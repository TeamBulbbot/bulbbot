...
export default class extends SubCommand {
	...
	async run(message: Message, args: string[]): Promise<void | Message> {
		...
		if ((await clearanceManager.getCommandOverride(<Snowflake>message.guild?.id, name)) !== undefined)
			return await message.channel.send(await this.client.bulbutils.translateNew("override_already_exists", message.guild?.id, {}));
		...
		await message.channel.send(await this.client.bulbutils.translateNew("override_create_success", message.guild?.id, { clearance }));
	}
}
