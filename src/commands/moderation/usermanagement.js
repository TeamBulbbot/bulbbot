...
module.exports = {
	...
	run: async (client, message, args) => {
		Role.findOne(
			...
			async (err, roles) => {
				if (message.member.roles.cache.has(roles.admin) || message.member.roles.cache.has(roles.moderator) || message.member.hasPermission("BAN_MEMBERS") || message.member.hasPermission("KICK_MEMBERS")) {
					...
					Infraction.find(
						...
						async (err, infs) => {
							...
							if (infs.length > 0) {
								descriptionBottom += `**Infractions: ** ${Math.floor(infs.length)}\n\n`;
								let moderator = await client.users.fetch(infs[infs.length - 1].moderatorID);
								descriptionBottom += `**Recent infraction: ** ${infs[infs.length - 1]._id}\n**Action: **${infs[infs.length - 1].action}\n**Reason: **${infs[infs.length - 1].reportReason}\n**Moderator: ** ${moderator.username}#${moderator.discriminator} \`\`(${moderator.id})\`\``;
							}
							let embed = new Discord.MessageEmbed()
								...
								.setAuthor(`${user.username}#${user.discriminator}`, user.avatarURL()).setDescription(`
${description}
${descriptionBottom}
		...
		`);
							...
							message.channel.send(embed).then(async (msg) => {
								...
								if (message.member.roles.cache.has(roles.admin) || message.member.hasPermission("BAN_MEMBERS")) msg.react(Emotes.actions.ban.replace(/\D/g, "")); // Ban
