		...
		switch (option) {
			...
			case "update":
				...
				Infraction.findOne(
					...
					async (err, inf) => {
						if (inf === null || inf === undefined) return message.channel.send(`Unable to find infraction with the id \`\`${args[1]}\`\` in **${message.guild.name}**`);
						let reason = args.slice(2).join(" ") || "No reason given";
						InfractionHandler.Update(args[1], message.guild.id, reason);
						message.channel.send(`${Emotes.other.wrench} Updated infraction \`\`${args[1]}\`\` in **${message.guild.name}**`);
						await SendLog.Mod_action(client, message.guild.id, `${Emotes.other.wrench} Infraction \`\`${args[1]}\`\` was edited by **${message.author.username}**#${message.author.discriminator} \`\`(${message.author.id})\`\`\n**Reason:** ${reason}`, "");
					}
				);
			...
			case "claim":
				...
				Infraction.findOne(
					...
					async (err, inf) => {
						...
						message.channel.send(`${Emotes.other.wrench} Updated infraction \`\`${args[1]}\`\` in **${message.guild.name}**`);
						SendLog.Mod_action(client, message.guild.id, `${Emotes.other.wrench} Infraction \`\`${args[1]}\`\` was claimed by **${user.username}**#${user.discriminator} \`\`(${user.id})\`\``, "");
					}
				);
			...
			case "remove":
				...
				Infraction.findOne(
					...
					async (err, infs) => {
						if (infs === null || infs === undefined) return message.channel.send(`Unable to find infraction with the id \`\`${args[1]}\`\` in **${message.guild.name}**`);
						...
						let reason = args.slice(2).join(" ") || "No reason given";
						SendLog.Mod_action(client, message.guild.id, `${Emotes.actions.unban} Infraction \`\`${args[1]}\`\` was removed by **${message.author.username}**#${message.author.discriminator} \`\`(${message.author.id})\`\` \n**Reason:** ${reason} `, "");
					}
				);
			...
			case "info":
				...
				Infraction.findOne(
					...
					async (err, inf) => {
						...
						let embed = new Discord.MessageEmbed().setColor(process.env.COLOR).setTimestamp().setThumbnail(user.avatarURL()).setDescription(
							`
								**${action}**
