	...
	const data = () => {
		...
		for (const command of client.commands.values()) {
			...
			const cmd = {
				...
				name_localizations: !command.devOnly && command.type === ApplicationCommandType.ChatInput ? translateSlashCommands(`sc_${command.name}_name`) : {},
				description_localizations: !command.devOnly && command.type === ApplicationCommandType.ChatInput ? translateSlashCommands(`sc_${command.name}_desc`) : {},
